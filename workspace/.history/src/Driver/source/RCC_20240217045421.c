...
RCC_WrongConfig_t RCC_enableClock(uint32_t CLKEnable)
{
    ...
    uint16_t volatile timeout = 6000;
...
RCC_WrongPLLConfig_t RCC_configurePLLCLK( uint32_t M, uint32_t N, uint32_t P, uint32_t Q)
{
    ...
    else
    {
        ...
        temp_q = RCC->RCC_PLLCFGR;
        temp_q &= ~(PLLQ_MASK);
        temp_q |= (Q << Q_SHIFT);
        RCC->RCC_PLLCFGR = temp_q;

        temp_p = RCC->RCC_PLLCFGR;
        temp_p &= ~(PLLP_MASK);
        temp_p |= (((P/2)-1) << P_SHIFT);
        RCC->RCC_PLLCFGR = temp_p;

        temp_n = RCC->RCC_PLLCFGR;
        temp_n &= ~(PLLN_MASK);
        temp_n |= (N << N_SHIFT);
        RCC->RCC_PLLCFGR = temp_n;

        temp_m = RCC->RCC_PLLCFGR;
        temp_m &= ~(PLLM_MASK);
        temp_m |= (M << M_SHIFT);
        RCC->RCC_PLLCFGR = temp_m;
