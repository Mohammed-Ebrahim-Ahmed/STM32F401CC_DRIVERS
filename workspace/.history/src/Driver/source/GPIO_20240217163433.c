...
#define GPIO_MODER_MASK 0x18
#define GPIO_OTYPER_MASK 0x4
...
GPIO_ErrorStatus_t GPIO_init(GPIO_Pin_t* GPIO_Pin)
{
    ...
    else if((((volatile uint32_t* const)GPIO_Pin->Port) != ((volatile uint32_t* const)GPIO_PORTA)) &&
            (((volatile uint32_t* const)GPIO_Pin->Port) != ((volatile uint32_t* const)GPIO_PORTB)) &&
            (((volatile uint32_t* const)GPIO_Pin->Port) != ((volatile uint32_t* const)GPIO_PORTA))  )
    {
    ...
    else
    {
        ...
        LOC_PUPD =  (((volatile GPIO_Peri_t* const)(GPIO_Pin->Port))->GPIO_PUPDR);

        LOC_Mode_MODER = ((GPIO_Pin->Mode) & GPIO_MODER_MASK) >> GPIO_MODER_BITS_SHIFT;
        LOC_Mode_OTYPER = ((GPIO_Pin->Mode) & GPIO_OTYPER_MASK) >> GPIO_OTYPER_BITS_SHIFT;
        LOC_Mode_PUPDR = ((GPIO_Pin->Mode) & GPIO_PUPDR_MASK);

        LOC_Mode  &= ~(GPIO_MODE_MASK << (GPIO_Pin->Pin * BIT_WIDTH_2));
        LOC_Speed &= ~(GPIO_SPEED_MASK << (GPIO_Pin->Pin * BIT_WIDTH_2));
        LOC_PUPD  &= ~(GPIO_PUPD_MASK << (GPIO_Pin->Pin * BIT_WIDTH_2));
        LOC_Type  &= ~(GPIO_TYPE_MASK << (GPIO_Pin->Pin ));
        
        
        LOC_Mode  |= ((LOC_Mode_MODER) << (GPIO_Pin->Pin * BIT_WIDTH_2));
        LOC_Speed |= ((GPIO_Pin->Speed) << (GPIO_Pin->Pin * BIT_WIDTH_2));
        LOC_PUPD  |= (LOC_Mode_PUPDR << (GPIO_Pin->Pin * BIT_WIDTH_2));
        LOC_Type  |= ((LOC_Mode_OTYPER) << (GPIO_Pin->Pin ));   
        ...
        ((volatile GPIO_Peri_t* const)(GPIO_Pin->Port))->GPIO_PUPDR = LOC_PUPD;
...
GPIO_ErrorStatus_t GPIO_SetPinValue(volatile void* const GPIO_Port, uint32_t GPIO_Pin, uint32_t GPIO_Val_Set)
{
    GPIO_ErrorStatus_t LOC_ErrorStatus = GPIO_isNotOk;
    ...
    else if ((((GPIO_Port)) != (volatile uint32_t* const)(GPIO_PORTA)) &&
             (((volatile uint32_t* const)(GPIO_Port)) != (volatile uint32_t* const)(GPIO_PORTB)) &&
             (((volatile uint32_t* const)(GPIO_Port)) != (volatile uint32_t* const)(GPIO_PORTC)))
    {
        LOC_ErrorStatus = GPIO_WrongPort;
    }
...
GPIO_ErrorStatus_t GPIO_GetPinValue(volatile void* const GPIO_Port, uint32_t GPIO_Pin, uint32_t* GPIO_Status)
{
    GPIO_ErrorStatus_t LOC_ErrorStatus = GPIO_isNotOk;
    ...
    else if ((((volatile uint32_t* const)(GPIO_Port)) != (volatile uint32_t* const)(GPIO_PORTA)) &&
             (((volatile uint32_t* const)(GPIO_Port)) != (volatile uint32_t* const)(GPIO_PORTB)) &&
             (((volatile uint32_t* const)(GPIO_Port)) != (volatile uint32_t* const)(GPIO_PORTC)))
    {
        LOC_ErrorStatus = GPIO_WrongPort;
