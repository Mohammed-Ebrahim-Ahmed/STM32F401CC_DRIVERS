...
GPIO_ErrorStatus_t GPIO_init(GPIO_Pin_t* GPIO_Pin)
{
    ...
    else
    {
        ...
        ((GPIO_Pin->Port))->GPIO_MODER = LOC_Mode;
        ((GPIO_Pin->Port))->GPIO_OSPEEDR = LOC_Speed;
        ((GPIO_Pin->Port))->GPIO_PUPDR = LOC_PUPD;
        ((GPIO_Pin->Port))->GPIO_OTYPER = LOC_Type;        

    }
    return LOC_ErrorStatus;
}

GPIO_ErrorStatus_t GPIO_SetPinValue(volatile uint32_t* const GPIO_Port, uint32_t GPIO_Pin, uint32_t GPIO_Val_Set)
{
    GPIO_ErrorStatus_t LOC_ErrorStatus = GPIO_isNotOk;
...
GPIO_ErrorStatus_t GPIO_GetPinValue(volatile uint32_t* const GPIO_Port, uint32_t GPIO_Pin, uint32_t* GPIO_Status)
{
    GPIO_ErrorStatus_t LOC_ErrorStatus = GPIO_isNotOk;
