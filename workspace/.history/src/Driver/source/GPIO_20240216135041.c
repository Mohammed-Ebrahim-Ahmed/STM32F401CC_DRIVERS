...
GPIO_ErrorStatus_t GPIO_init(GPIO_Pin_t* GPIO_Pin)
{
    ...
    else
    {
        ...
        LOC_Mode =  ((volatile GPIO_Peri_t* const)GPIO_PORTA)->GPIO_MODER;
        LOC_Speed = ((volatile GPIO_Peri_t* const)GPIO_PORTB)->GPIO_OSPEEDR;
        LOC_Type =  ((volatile GPIO_Peri_t* const)GPIO_PORTA)->GPIO_OTYPER;
...
GPIO_ErrorStatus_t GPIO_SetPinValue(uint32_t GPIO_Port, uint32_t GPIO_Pin, uint32_t GPIO_Val_Set)
{
    ...
    else if((GPIO_Port != GPIO_PORTA_SEL) &&
    ...
    else
    {
        ...
        {
            GPIO[GPIO_Port]->GPIO_BSRR |= (GPIO_Bit_MASK << GPIO_Pin << GPIO_SHIFT_HIGH);
        }
        else
        {
            GPIO[GPIO_Port]->GPIO_BSRR |= (GPIO_Bit_MASK << GPIO_Pin << GPIO_SHIFT_LOW);
        }
    }
    return LOC_ErrorStatus;
}
...
GPIO_ErrorStatus_t GPIO_GetPinValue(uint32_t GPIO_Port, uint32_t GPIO_Pin, uint32_t* GPIO_Status)
{
    ...
    {
        *GPIO_Status = ((GPIO[GPIO_Port]->GPIO_IDR >> GPIO_Pin) & GPIO_Bit_MASK);
    }
    return LOC_ErrorStatus;
}
/*********************************************************************************************************************************************/