...
SWITCH_errorStatus_t Get_Switch_State(uint32_t SWITCH, volatile uint32_t* state)
{
    SWITCH_errorStatus_t LOC_errorStatus = SWITCH_NotOk;
    volatile uint8_t LOC_Counter = 0;
    volatile uint8_t flag = 0;
    volatile uint32_t LOC_state = 3;
    volatile uint8_t shift = 1;
    ...
    if(flag)
    {
        ...
        value |= ~(SWITCHS[SWITCH].Mode >> shift);      
